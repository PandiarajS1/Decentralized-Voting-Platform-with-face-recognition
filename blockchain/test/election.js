...
contract("Election", function (accounts) {
  ...
  it("throws an exception for double voting", async function () {
    ...
    // Check that vote count has not increased
    const candidate1 = await electionInstance.candidates(1);
    assert.equal(candidate1[3], 1, "Candidate 1 did not receive extra votes");

    const candidate2 = await electionInstance.candidates(2);
    assert.equal(candidate2[3], 1, "Candidate 2 did not receive extra votes");
  });
});
